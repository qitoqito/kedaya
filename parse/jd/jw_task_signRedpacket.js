...
class Main extends Template {
    ...
    async main(p) {
        ...
        let s = await this.algo.curl({
                'url': `https://api.m.jd.com/signTask/doSignTask?client=apple&clientVersion=7.16.280&functionId=SignComponent_doSignTask&appid=hot_channel&loginWQBiz=signcomponent&loginType=2&body={"activityId":"10004"}`,
                cookie,
                headers: {
                    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15F79 MicroMessenger/8.0.15(0x18000f2e) NetType/WIFI Language/zh_CN',
                    referer: 'https://servicewechat.com/wx91d27dbf599dff74/616/page-frame.html'
        ...
        if (!info.completionFlag) {
            let ss = await this.algo.curl({
                    'url': `https://api.m.jd.com/scanTask/startScanTask?client=apple&clientVersion=7.17.300&functionId=SignComponent_doScanTask&appid=hot_channel&loginWQBiz=signcomponent&loginType=2&body={"itemId":"${info.itemId}","activityId":"10004","scanAssignmentId":"${info.scanAssignmentId}","actionType":1}`,
                    // 'form':``,
                    cookie,
                    headers: {
                        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15F79 MicroMessenger/8.0.15(0x18000f2e) NetType/WIFI Language/zh_CN',
            ...
            let r = await this.algo.curl({
                    'url': `https://api.m.jd.com/scanTask/startScanTask?client=apple&clientVersion=7.17.300&functionId=SignComponent_doScanTask&appid=hot_channel&loginWQBiz=signcomponent&loginType=2&body={"itemId":"${info.itemId}","activityId":"10004","scanAssignmentId":"${info.scanAssignmentId}","actionType":0}`,
                    // 'form':``,
                    cookie,
                    headers: {
                        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15F79 MicroMessenger/8.0.15(0x18000f2e) NetType/WIFI Language/zh_CN',
