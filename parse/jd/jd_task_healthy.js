...
class Main extends Template {
    ...
    async main(p) {
        ...
        let headers = {
            ...
            'jdgs': `{"b1":"${this.uuid(8)}-${this.uuid(4)}-${this.uuid(4)}-${this.uuid(4)}-${this.uuid(12)}","b2":"2.2.4_1","b3":"2.1","b4":"","b5":"${this.sha1(p.cookie)}","b7": ${new Date().getTime().toString()},"b6":"${this.sha1(p.user)}"}`
        }
