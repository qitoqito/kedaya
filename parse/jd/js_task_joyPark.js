...
class Main extends Template {
    ...
    async prepare() {
        this.linkId = "LsQNxL7iWDlXUs6cFl-AAg"
        this.clientVersion = "4.9.2"
        this.build = "167863"
    ...
    async main(p) {
        ...
        let reports = setInterval(async function f() {
            ...
            let report = await self.algoCurl({
                    'url': `functionId=gameHeartbeat&body={"businessCode":1,"linkId":"${this.linkId}"}&t=${new Date().getTime()}&appid=activities_platform&client=ios&clientVersion=${this.clientVersion}&cthr=1&uuid=0721076da75ec3ea8e5f481e6d68bb4b7420c38d&build=${this.build}&screen=390*844&networkType=wifi&d_brand=iPhone&d_model=iPhone13,3&lang=zh_CN&osVersion=15.1.1&partner=&eid=eidId7f9812189s4Ywiz164KTQqoeSyoW1uZwmMItV216n8pCJ26eJPEqZb5n8VkyLjW71hRQ6fhLku8USG3jg%2BHtZ7ecv%2BJ2CWEYpUd99P1GvH7bppT`,
                    cookie,
                    delay: 10,
        ...
        try {
            ...
            if (this.dict[p.user].unlock && joys.length>0) {
                ...
                for (let i = 0; i<min; i++) {
                    ...
                    console.log('移动牛牛去工作...', this.haskey(move, 'success'))
                }
            }
            ...
            if (this.dict[p.user].joy[i].length>1) {
                ...
                for (let k = 0; k<spl.length; k++) {
                    ...
                    if (this.haskey(merge, 'data.joyVO.id')) {
                        ...
                        joyss[i] = spl[k + 1] || []
                        console.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, JSON.stringify(joyss))
                    }
                    // console.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, this.dict[p.user].joy[parseInt(i) + 1])
                }
            }
        }
        if (isChange>0) {
            ...
            for (let i = 21; i<25; i++) {
                if (!this.haskey(joy, `${i}.0`)) {
                    ...
                    if (joyInfo && this.dict[p.user].coin>0 && joyInfo.consume>0 && joyInfo.consume<this.dict[p.user].coin) {
                        ...
                        let buy = await this.algoCurl({
                                'url': `https://api.m.jd.com/`,
                                'form': `functionId=joyBuy&body={"level":${i},"linkId":"${this.linkId}"}&t=1681023762664&appid=activities_platform&client=ios&clientVersion=${this.clientVersion}&cthr=1&uuid=bd573a56457eba54de7a6c0787c1fbb4fde28eb2&build=${this.build}&screen=375*667&networkType=wifi&d_brand=iPhone&d_model=iPhone8,1&lang=zh_CN&osVersion=13.7&partner=&eid=eidI08a2812293saa9h49%2BwmQbOdWcGqiWsHQ2vYen7SFhReSdDTvgVd9CzRHKrkpiAq6WU2YgJf8TchQcbWEAdBOCTuiYEdV5DxTHW0eO1PylPf2QAx`,
                                cookie,
                                algo: {
                                    'appId': 'ffb36'
                                }
                        ...
                        console.log(`没有足够的金币可以购买等级${i}的牛牛...`)
                        break
                    }
                }
            }
                    ...
                    for (let i in this.dict[p.user].joy) {
                        if (this.dict[p.user].joy[i] && this.dict[p.user].joy[i].length>1 && this.dict[p.user].joy[i].length % 2 == 0
                            ...
                            for (let k of spl) {
                                ...
                                await this.joyList(p)
                                // console.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, this.dict[p.user].joy[parseInt(i) + 1])
                                console.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, JSON.stringify(this.dict[p.user].joy))
                            }
                        }
                    }
                }
    ...
    async baseInfo(p) {
        ...
        if (this.haskey(baseInfo, 'data.level') == 30) {
            let joyRestart = await this.algoCurl({
                    'url': `https://api.m.jd.com/`,
                    'form': `functionId=joyRestart&body={"linkId":"${this.linkId}"}&t=1681012535084&appid=activities_platform&client=ios&clientVersion=${this.clientVersion}&cthr=1&uuid=bd573a56457eba54de7a6c0787c1fbb4fde28eb2&build=${this.build}&screen=375*667&networkType=wifi&d_brand=iPhone&d_model=iPhone8,1&lang=zh_CN&osVersion=13.7&partner=&eid=eidI08a2812293saa9h49%2BwmQbOdWcGqiWsHQ2vYen7SFhReSdDTvgVd9CzRHKrkpiAq6WU2YgJf8TchQcbWEAdBOCTuiYEdV5DxTHW0eO1PylPf2QAx`,
                    cookie,
                    algo: {
                        'appId': '4abce'
                    }
