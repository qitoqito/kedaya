...
class Main extends Template {
    ...
    async prepare() {
        ...
        for (let cookie of this.cookies['help']) {
            let s = await this.curl({
                    ...
                    'form': `functionId=party_inviteWindow&body={"showAssistorsSwitch":true}&client=wh5&clientVersion=1.0.0&appid=spring_h5`,
                    cookie
                }
            )
            try {
    ...
    async main(p) {
        ...
        // let s = await this.curl({
        //         'url': `https://api-x.m.jd.com/`,
        //         'form': `functionId=party_assistWindow&body={"inviteCode":"${p.inviter.inviteCode}"}&client=wh5&clientVersion=1.0.0&appid=spring_h5`,
        //         cookie
        //     }
        // )
        ...
        let z = await this.algo.curl({
                'url': `https://api-x.m.jd.com/`,
                form: `functionId=party_assist&body={"inviteCode":"${p.inviter.inviteCode}","uuid":"sX88mRXGZ63KidZr","sv":"${this.sha256(p.user)}"}&client=wh5&clientVersion=1.0.0&appid=spring_h5`,
                // 'form':``,
                cookie
            }
        )
        console.log(z)
        if (z.data.bizMsg.includes('不需要')) {
